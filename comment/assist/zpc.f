...
c$$$        COMMON /para1/ mul
c$$$cc      SAVE /para1/
c$$$        common /para4/ iftflg, ireflg, igeflg, ibstfg
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$c$$$        iftflg:switch for the generation of particle formation time.
c$$$c$$$        (粒子的信息时间)
c$$$c$$$        = 0 : generate formation time.
c$$$c$$$        = 1 : read in formation time from zpc.inp
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$c$$$        ireflg:switch to read initial conditions from zpc.inp.
c$$$c$$$        (读取初始条件)
c$$$c$$$        = 0 : read the initial conditions
c$$$c$$$        = 1 : do not rad the initial conditions.
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$c$$$        igeflg:switch to generate initial conditions.
c$$$c$$$        (产生初始条件)
c$$$c$$$        = 0 : do not generate initial conditions.
c$$$c$$$        = 1 : generate initial conditions
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$c$$$        ibstfg:choice of global frame.
c$$$c$$$        = 0 : the global frame is the collider center of mass frame
c$$$c$$$        for Au and Au collisions.
c$$$c$$$        = 1 : means the global frame is taken to be the target frame.
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$cc      SAVE /para4/
c$$$        SAVE   
c$$$
...
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$c$$$        ireflg = 1, igeflg = 0, ibstfg = 0.
c$$$c$$$        即下面的三个调用一个都不进行.
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$c$$$        write(*,*) " ireflg = ", ireflg
c$$$c$$$        write(*,*) " igeflg = ", igeflg
c$$$c$$$        write(*,*) " ibstfg = ", ibstfg
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$c$$$         WW       WW        WW RRRRRRR  II TTTTTTTTTT EEEEEE
c$$$c$$$          WW     WW WW     WW  RR  RR   II     TT     EE
c$$$c$$$           WW   WW   WW   WW   RRRRR    II     TT     EEEEEE
...
c$$$c$$$             WW        WW      RR  RR   II     TT     EEEEEE
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        if (ireflg .eq. 0) call readi
c$$$        if (igeflg .ne. 0) call genei
c$$$        if (ibstfg .ne. 0) call boosti
c$$$
c$$$        return
c$$$        end
c$$$c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$已阅
...
c$$$        double precision field(9)
c$$$        common /para1/ mul
c$$$cc      SAVE /para1/
c$$$        common /para3/ nsevt, nevnt, nsbrun, ievt, isbrun
c$$$cc      SAVE /para3/
...
c$$$cc      SAVE /prec1/
c$$$        SAVE   
c$$$        do 1001 i = 1, MAXPTN
c$$$           if (ievt .ne. 1 .and. i .eq. 1) then
c$$$              ityp0(i) = ntyp
c$$$              gx0(1) = field(1)
c$$$              gy0(1) = field(2)
c$$$              gz0(1) = field(3)
...
c$$$              mul = 1
c$$$           else
c$$$ 900              read (27, *, end = 1000) neve, ntyp, field
c$$$              if (neve .lt. nsevt) goto 900
c$$$              if (neve .gt.
c$$$     &           nsevt + ievt - 1) goto 1000
c$$$              ityp0(i) = ntyp
c$$$              gx0(i) = field(1)
c$$$              gy0(i) = field(2)
c$$$              gz0(i) = field(3)
...
c$$$        common /para1/ mul
c$$$cc      SAVE /para1/
c$$$        common /para2/ xmp, xmu, alpha, rscut2, cutof2
c$$$cc      SAVE /para2/
c$$$        common /para3/ nsevt, nevnt, nsbrun, ievt, isbrun
c$$$cc      SAVE /para3/
c$$$        common /para5/ iconfg, iordsc
c$$$cc      SAVE /para5/
c$$$        COMMON /prec1/GX0(MAXPTN),GY0(MAXPTN),GZ0(MAXPTN),FT0(MAXPTN),
c$$$     &       PX0(MAXPTN), PY0(MAXPTN), PZ0(MAXPTN), E0(MAXPTN),
c$$$     &       XMASS0(MAXPTN), ITYP0(MAXPTN)
c$$$cc      SAVE /prec1/
c$$$        common /prec5/ eta(MAXPTN), rap(MAXPTN), tau(MAXPTN)
c$$$cc      SAVE /prec5/
c$$$        common /lor/ enenew, pxnew, pynew, pznew
c$$$cc      SAVE /lor/
c$$$        common /rndm3/ iseedp
c$$$cc      SAVE /rndm3/
c$$$        SAVE   
c$$$
c$$$        external ran1
...
c$$$        r0 = 5d0
c$$$        tau0 = 0.1d0
c$$$        deta = etamax - etamin
c$$$
c$$$        do 1001 i = mul + 1, mul + incmul
c$$$           ityp0(i) = 21
c$$$           xmass0(i) = xmp
c$$$           call energy(e, temp)
c$$$           call momntm(px, py, pz, e)
c$$$c     7/20/01:
c$$$c           e = sqrt(e ** 2 + xmp ** 2)
c$$$           e = dsqrt(e ** 2 + xmp ** 2)
c$$$           if (iconfg .le. 3) then
c$$$              eta(i) = etamin + deta * ran1(iseed)
c$$$              bex = 0d0
c$$$              bey = 0d0
c$$$              bez = -tanh(eta(i))
c$$$              call lorenz(e, px, py, pz, bex, bey, bez)
c$$$              px0(i) = pxnew
c$$$              py0(i) = pynew
c$$$              pz0(i) = pznew
c$$$              e0(i) = enenew
...
c$$$ 1001   continue
c$$$
c$$$        do 1002 i = mul + 1, mul + incmul
c$$$           if (iconfg .le. 3) then
c$$$              gz0(i) = tau0 * sinh(eta(i))
c$$$              ft0(i) = tau0 * cosh(eta(i))
c$$$              if (iconfg .eq. 1) then
c$$$                 call posit1(x, y, r0)
c$$$                 gx0(i) = x + px0(i) * ft0(i)/e0(i)
c$$$                 gy0(i) = y + py0(i) * ft0(i)/e0(i)
c$$$              else if (iconfg .eq. 2 .or. iconfg .eq. 3) then
c$$$                 call posit2(x, y)
c$$$                 gx0(i) = x
c$$$                 gy0(i) = y
c$$$              end if
...
c$$$        mul = mul + incmul
c$$$            
c$$$c       check if it's necessary to adjust array size 'adarr'
c$$$            if (mul .ge. MAXPTN .or. mul .eq. 0) then
c$$$           print *, 'event',ievt,'has',mul,'number of gluon',
c$$$     &          'adjusting counting is necessary'
c$$$           stop 'adarr'
c$$$        end if
c$$$        
c$$$        return
...
c$$$        bez = - tanh(centy)
c$$$        
c$$$c       save data for many runs of the same initial condition           
c$$$        do 1001 i = 1, mul
c$$$           px1 = gx0(i)
c$$$           py1 = gy0(i)
c$$$           pz1 = gz0(i)
c$$$           e1 = ft0(i)
c$$$           call lorenz(e1, px1, py1, pz1, bex, bey, bez)
c$$$           gx0(i) = pxnew
c$$$           gy0(i) = pynew
c$$$           gz0(i) = pznew
c$$$           ft0(i) = enenew
...
******************************************************************************
        ...
        subroutine inirun                  !!!!!!!!!!!!!!!已阅。
        SAVE   
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        prec2: main particle record that has particle types, the
c$$$        positions at formation times, the corresponding formation times
c$$$        and the four momenta of the particles, plus the masses of the
c$$$        particles.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

c       sort prec2 according to increasing formation time
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        ftime : this subroutine generates formation time for the particles
c$$$        indexing ft(i), input e(i), output ft(i), indx(i)
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        call ftime
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        inirec : 对数据块PREC2的数据进行赋值,如果isoft=5，好像还能
c$$$        对初始的析出条件进行赋值。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        call inirec
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        对每一个粒子的next(i), last(i), icsta(i), nic(i), icels(i)
c$$$        数组进行初始化，这些数组在记录什么??
c$$$        对icel(i1, i2, i3)的数组进行初始化。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        call iilist
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        貌似就是对能损的数组进行初始化。
...
c$$$             WW        WW      RR  RR   II     TT     EEEEEE
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<
            write(*,*) "ireflg = 1", ireflg
            write(*,*) "igeflg = 0", igeflg
            write(*,*) "ibstfg = 0", ibstfg
            write(*,*) "iftflg = 0", iftflg
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$         WW       WW        WW RRRRRRR  II TTTTTTTTTT EEEEEE
c$$$          WW     WW WW     WW  RR  RR   II     TT     EE
c$$$           WW   WW   WW   WW   RRRRR    II     TT     EEEEEE
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
 ...
 1002         continue
              goto 150
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        看到上面的150,因该猜到下面的程序是初始isoft=1的ft0(i)的数据。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
           else
c     5/01/01-end

c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        对ft0(i)进行初始化,默认初始化是相当大的.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
           do 1003 i = 1, MAXPTN
              ft0(i) = tlarge
 1003      continue
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        如果多重数大于1,调用index1程序。注意index1函数的调用在多重
c$$$        数的循环之内。我们一般肯定调用才函数，而不进入到下面的else.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        if (mul .gt. 1) then
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        下面的四行条件语句只在isoft=5时才进行调用。也就是说不调用。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        if(isoft.eq.5) then
           itlast=0
           call inifrz
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        下面的循环遍历所有的粒子，对数据块PREC2的数据进行赋值。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        do 1001 i = 1, mul
...
c$$$        = 6: a formation, collision between particles and a wall collision
c$$$             at the same time.
c$$$        ictype是操作的类型,当ictype的偶数的时候，进入到下面的条件
c$$$        语句中。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        if (mod(ictype, 2) .eq. 0) then
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        savrec程序的作用 : 用prec2的数据给prec3的数据进行初始化(赋值)。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$         WW       WW        WW RRRRRRR  II TTTTTTTTTT EEEEEE
c$$$          WW     WW WW     WW  RR  RR   II     TT     EE
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        在t1时间决定有哪些粒子进行冻结和系出,并且给出粒子的冻结
c$$$        的OSCAR信息.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      if(isoft.eq.5) then
         call local(t1)
...
c$$$             WW        WW      RR  RR   II     TT     EEEEEE
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$           write (999, 1233) 'iscat=', iscat, 'jscat=', jscat,
           write (9999, *) 'iscat=', iscat, ' jscat=', jscat,
     1          ityp(iscat), ityp(jscat)
           write (9999, 1233) 'iscat=', max(indx(iscat), indx(jscat)),
     &          'jscat=', min(indx(iscat), indx(jscat))

           write (9999, 1234) ' icolln=', icolln, 't=', t
...
c$$$        = 6: a formation, collision between particles and a wall collision
c$$$             at the same time.
c$$$        ICONFIG:几何位形和空间格点的划分.
c$$$        = 1: the system is undergoing 3-d expansion.
c$$$        = 2: the system is undergoing 1-d expansion with space cell
c$$$        = 3: the system is undergoing 1-d expansion without space cell
c$$$        = 4: the system is confined in a box with space cell division
c$$$        = 5: the system is confined in a box without space cell
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        if (iconfg .eq. 1
     &     .or. iconfg .eq. 2
     &     .or. iconfg .eq. 4) then
...
c$$$             WW        WW      RR  RR   II     TT     EEEEEE
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$         check icsta(i): 0 with f77 compiler
           write(9998,*) 'BB:ictype,t1,iscat,jscat,icsta(i)=',
     1          ictype,t1,iscat,jscat,icsta(iscat)
           
c$$$       check collision time table error 'tterr'
c$$$       to avoid out-of-bound error in next():
           if (jscat .ne. 0 .and. next(jscat) .ne. iscat)
     &        then
              write(9998,*) 'iscat=', iscat, 'jscat=', jscat,
...
c           end if
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        cellre程序的作用：用来改变碰撞粒子的cell with the wall。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$             WW        WW      RR  RR   II     TT     EEEEEE
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<
             write(95,*) 'informationid: px, py, pz, m, x, y, z, t'
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$         WW       WW        WW RRRRRRR  II TTTTTTTTTT EEEEEE
c$$$          WW     WW WW     WW  RR  RR   II     TT     EE
c$$$           WW   WW   WW   WW   RRRRR    II     TT     EEEEEE
...
c
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        由ifmpt，ft(ifmpt)和t1的数值得出ictype的数值。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        if (ifmpt .le. mul) then
...
cc      SAVE /ilist1/
 ...
 10           continue
        ...
        subroutine scat(t, iscat, jscat)   !!!!!!!!!!!!!!!!!!!已阅。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        用来计算两个粒子的散射。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c       this subroutine is used to calculate the 2 particle scattering
        ...
        subroutine newpos(t, i)              !!!!!!!!!!!!!!!!!已阅。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        计算两个粒子的散射从而得到新的位置信息。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c       this subroutine is used to calculate the 2 particle scattering
c       get new position
...
c$$$             WW        WW      RR  RR   II     TT     EEEEEE
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<
        write(9997,*) "dt1 gx(i) gy(i) gz(i) and ft(i) eta(i) tau(i)"
        write(9997,*) dt1, gx(i), gy(i), gz(i), ft(i), eta(i), tau(i)
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$         WW       WW        WW RRRRRRR  II TTTTTTTTTT EEEEEE
c$$$          WW     WW WW     WW  RR  RR   II     TT     EE
c$$$           WW   WW   WW   WW   RRRRR    II     TT     EEEEEE
...
c$$$             WW        WW      RR  RR   II     TT     EEEEEE
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<
             write(9996,*)"randomize the interaction (iff) : ", iff
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$         WW       WW        WW RRRRRRR  II TTTTTTTTTT EEEEEE
c$$$          WW     WW WW     WW  RR  RR   II     TT     EE
c$$$           WW   WW   WW   WW   RRRRR    II     TT     EEEEEE
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        利用两个粒子的信息对px1, py1, pz1, e1, x1, y1, z1, t1
c$$$        和px2, py2, pz2, e2, x2, y2, z2, t2进行赋值。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        px1 = px(iscat)
        py1 = py(iscat)
        pz1 = pz(iscat)
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        下面的程序由iconfg的数值对x2, y2, 和z2进行赋值。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        if (iconfg .eq. 1) then
           x2 = gx(jscat)
           y2 = gy(jscat)
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        上面的程序由iconfg的数值对x2, y2, 和z2进行赋值。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        t2 = ft(jscat)
ctrans
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        本程序getht用来得到传进去的参数that的数值。但是that是什么？
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        cropro(...) : 利用传进来的vx1, vy1, vz1, vx2, vy2, vz2.
c$$$        计算vx3, vy3, vz3.并将数值存到/cprod/中。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        call cropro(x1-x2, y1-y2, z1-z2, px1, py1, pz1)

c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        xnormv ： 用来得到归一化的vx, vy, vz.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        zprota(...) : 矢量(v1,v2,v3)绕着单位矢量(xn1,xn2,xn3)旋转
c$$$                     theta角度。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     &        rap1 .gt. centy - 0.5d0)) then
              write (9995,*) sqrt(ft(iscat) ** 2 - gz(iscat) ** 2), rts2
           end if
           if ((rap2 .lt. centy + 0.5d0 .and.
     &        rap2 .gt. centy - 0.5d0)) then
              write (9995,*) sqrt(ft(jscat) ** 2 - gz(jscat) ** 2), rts2
           end if
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$         WW       WW        WW RRRRRRR  II TTTTTTTTTT EEEEEE
c$$$          WW     WW WW     WW  RR  RR   II     TT     EE
...
ctransend
        ...
        subroutine getht(iscat, jscat, pp2, that)    !!!!!!!!!!已阅。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        本程序的目的就是得到传进来的最后一个参数的数值。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        xmu为:screening mass.
c$$$        xmp : mass of the particles.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        xmu2 = (hbarc * xmu) ** 2
        xmp2 = xmp ** 2
        xm2 = xmu2 + xmp2
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        subroutine ulist1(l, t)                  !!!!!!!!!!!!!!!已阅。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        ulist1(l, t) : 更新相互作用列表，当该粒子是disturbed??????
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c       otherwise wallc will change icsta
        ...
        subroutine wallc(i, i1, i2, i3, t, tmin)     !!!!!!!!!!!!已阅。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        这个程序计算下次的碰撞的时间with wall对于粒子i.
c$$$        input 粒子的标记 i, t
c$$$        输出粒子的碰撞时间with wall, icsta(i) 墙的碰撞信息。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        这个程序用来得出墙的碰撞时间当粒子在cube内，设定粒子的icsta,
c$$$        输出tmin, icsta(i),其中icsta并没有最终的设定，我们需要进一步的判
c$$$        断。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        根据iconfg的数值和3，5的关系，调用wallc2(...)函数。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        else if (iconfg .eq. 3 .or. iconfg .eq. 5) then
        ...
        subroutine wallc1(i, i1, i2, i3, t, tmin)      !!!!!!!!!!已阅。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        这个程序用来得出墙的碰撞时间当粒子在cube内，设定粒子的icsta,
c$$$        输出tmin, icsta(i),其中icsta并没有最终的设定，我们需要进一步的判
c$$$        断。
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        根据tmin和t1, t2, t3的关系对icsta(i)进行赋值。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        ICSTA(I):the operation type for particle I.
c$$$        = 0: an ordinary collision.
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        由v1p和v1,v2p和v2,v3p和v3的关系确定t1, t2, and t3.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        根据t1, t2 and t3谁是最小值，分别对icsta(i)进行赋值。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        if (tmin .eq. t1) then
           if (v1p .gt. (i1 - 5) * v1) then
              icsta(i) = 101
        ...
        subroutine wallc2(i, i1, i2, i3, t, tmin)        !!!!!!!!!!!已阅。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        this subroutine is used to get wall collision time
c$$$        when particle is inside the cube, it sets the icsta at the same time
c$$$        input i,i1,i2,i3,t
...
c       itest is a flag to make sure the 111111 cell is checked only once
 ...
 1003   continue
        ...
        subroutine chkcel(il, i1, i2, i3, t, tmin, nc)       !!!!!!!已阅
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
cc      SAVE /ilist5/
        ...
        subroutine ud2(i, j, t, tmin, nc)          !!!!!!!!已阅。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        if (iconfg .eq. 1) then
           ...
           ipic = 11
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        iconfig = 2 或 iconfig = 3系统正在经历一维的扩张.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
cd           t2 = ft(i)
           ...
           ipic = 12
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        iconfig = 4 或 iconfig = 5系统被束缚在一个盒子里面.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        zpca1b(rapi, et, ian):貌似就是更新能损之类的东西的.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
              end if
 1002      continue
        else
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        子程序zpca1c(p0, p1, p2, p3, ian)求解em(i, j, ian)张量的数值.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
              end if
 1003      continue
        end if
...
c$$$        t: the current operation time.
c$$$        iopern: 当前的操作数.
c$$$        icolln: 当前粒子完成的碰撞的数量.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
cc      SAVE /ilist6/
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        该数据块用来记录dEt/dy, dEt/dN, dN/dy.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        common /ana2/
     &     det(12), dn(12), detdy(12), detdn(12), dndy(12),
     &     det1(12), dn1(12), detdy1(12), detdn1(12), dndy1(12),
...
c$$$c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$           if (ian .eq. 10) then
c$$$cd              write (10, *) t, det2(ian)
c$$$           end if
c$$$           if (ian .eq. 11) then
c$$$cd              write (11, *) t, det2(ian)
c$$$           end if
c$$$           if (ian .eq. 12) then
c$$$cd              write (12, *) t, det2(ian)
c$$$           end if
cdtransend
...
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        前四个参数貌似是动量空间的四动量.第五个参数粒子的索引.
c$$$        此段程序貌似只更新了数组em(i, j, ian).
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c$$$        iconfig小于3为:
c$$$        = 1: 系统正在经历三维的扩张.
c$$$        = 2: 系统正在经历一维的扩张而且有space cell division的划分.
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        if (iconfg .le. 3) then
           call zpca2a
        else
...
******************************************************************************
      ...
      subroutine lorenz(energy, px, py, pz, bex, bey, bez)     !!!!!!!!已阅
...
c         gam = 1.d0 / sqrt(1.d0 - beta2)
     ...
     &        + (1.d0 + (gam - 1.d0) * bez ** 2 / beta2) * pz    
      ...
      subroutine index1(n, m, arrin, indx)      !!!!!!!!!!!!!!!已阅
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        call index1(MAXPTN，MUL，FT0，INDX)
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c        ftime1 = aa * sqrt(1d0 / ran1(iseed) - 1d0)
      ...
      subroutine cropro(vx1, vy1, vz1, vx2, vy2, vz2)        !!!!!已阅。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        利用传进来的vx1, vy1, vz1, vx2, vy2, vz2.计算vx3, vy3, vz3.
c$$$        并将数值存到/cprod/中。
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
c      subroutine rotate(xn1, xn2, xn3, theta, v1, v2, v3)
      subroutine zprota(xn1, xn2, xn3, theta, v1, v2, v3)     !!!!!!已阅。
cbz1/29/99end
c$$$      >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c$$$        zprota(...) : 矢量(v1,v2,v3)绕着单位矢量(xn1,xn2,xn3)旋转
c$$$                     theta角度。
